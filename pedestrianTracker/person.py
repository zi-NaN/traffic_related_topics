...
match the moving persons and draw rectangles around it
'''
import cv2
import time
import math
...
class Person:
    ...
    def match(self, rect):
        ...
        return max(0, min(match, 100)) # always the same as match??
    ...
    def set_rect(self, rect):
        ...
        if self._center is not None:
            ...
            distance = x_center - self._center[0] # only cal the horizontal speed 
                                                # and it contains the direction
            
            latest_speed = distance/time_interval
            self.pspeed = self.pspeed * (1-self.alpha) + latest_speed * self.alpha if self.pspeed != 0 else latest_speed
    ...
    def draw(self, frame):
        ...
        color = (self._color[2], self._color[1], self._color[0], alpha)
